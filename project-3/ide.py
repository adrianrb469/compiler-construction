...
with col2:
    ...
    if code:
        tac, table, errors, _, assembly = compile(code)

        # Create tabs for the TAC output and Symbol Table
        tab1, tab2, tab3 = st.tabs(
            ["Three-Address Code", "Symbol Table", "MIPS Assembly"]
        )
        ...
        with tab3:
            # Display MIPS Assembly if no errors
            if errors is None:
                st.subheader("MIPS Assembly")
                st.code(assembly, language="text")

                # Add a download button for the MIPS assembly code
                st.download_button(
                    label="Download MIPS Assembly",
                    data=assembly,
                    file_name="assembly.s",
                    mime="text/plain",
                )
